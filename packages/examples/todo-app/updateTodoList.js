  ...
  element.innerHTML = /*html*/`
    <ul class="divide-y divide-gray-200">
      ${todos.length === 0 
        ...
        `
        : todos.map(todo => /*html*/`
          <li class="py-4 flex items-center justify-between ${todo.completed ? 'bg-gray-50' : ''}">
            <div class="flex items-center flex-1">
              <input
                ...
                data-id="${todo.id}"
              >
              <label class="ml-3 block text-gray-900 flex-grow ${todo.completed ? 'line-through text-gray-500' : ''}">${todo.text}</label>
            </div>
            <button
