...
export class SignalList<T> extends HTMLElement {
  ...
  private handleMixedArray(newItems: unknown[]): void {
    ...
    newItems.forEach((item, index) => {
      ...
      if (existingElement) {
        ...
        console.log('Updating existing element index', { index, item });
  ...
  private walkElements(element: Element, callback: (element: Element) => void): void {
    // Process current element
    callback(element);

    // Recursively process child elements
    Array.from(element.children).forEach(child => this.walkElements(child, callback));
  }
