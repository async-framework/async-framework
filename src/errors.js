...
export function createSchedulerErrorBridge(getOnError) {
  return (error, job) => {
    const scope = job?.scope;
    reportAsyncError({
      target: typeof scope?.dispatchEvent === "function" ? scope : undefined,
      error,
      onError: getOnError(),
      context: typeof job?.key === "string" ? { schedulerJob: job.key } : undefined
    });
  };
}
